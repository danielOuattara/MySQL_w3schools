...
```sql
CREATE DATABASE w3schools_demo_db;
```

Up until now in this tutorial we have worked with a very
small and simple tables in our MySQL database.

Now we want to create more tables with more content to be
able to demonstrate more database features.

We will create these 6 tables in our MySQL database:
...
Below we have listed all the SQL statements you need to create those
tables, with content.

You are not required to create the tables on your own system to
continue with this tutorial, but you might better understand how
MySQL and SQL statements work.
...
```sql
-- log in as root : 
GRANT ALL PRIVILEGES ON w3schools_north_wind.* TO 'daniel'@'localhost';
-- log out root 

-- log in as daniel 
mysql -u daniel -p

CREATE DATABASE w3schools_north_wind;
USE w3schools_north_wind;

-- continue the project as daniel logged in
```

Make sure you are connected to the database. If not, follow the steps
in the SQL Shell chapter of this tutorial.
...
```sql
INSERT INTO 
  products (id, product_name, category_id, unit, price)
VALUES
  ...
  (77, 'Original Frankfurter gröne Soae', 2, '12 boxes', 13);
```
...
We will also add a table called `testproducts` for demonstration
purposes later in the tutorial.
